...
def _speak_android_native(text: str) -> bool:
    """Use the host Android app's TextToSpeech bridge when running embedded."""
    if os.environ.get("HERMES_ANDROID_EMBEDDED") != "1":
        return False
    try:
        from java import jclass  # type: ignore

        activity = jclass("com.sesaloy.hermes.MainActivity")
        return bool(activity.speakTextFromPython(text, "zh-CN", 30000))
    except Exception as exc:
        logger.debug("Android native TTS bridge failed: %s", exc)
        return False
...
def speak_text(text: str) -> None:
    ...
    try:
        ...
        if not tts_text:
            return
        if _speak_android_native(tts_text):
            _debug("speak_text: Android native TTS completed")
            return
